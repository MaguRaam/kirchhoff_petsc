...
PetscErrorCode ComputeEmissionDistanceAndCos(Vec Tauq, Vec Rq, Vec Cosq, PetscReal to, Point xo, PetscReal c){
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"q_points.dat",FILE_MODE_READ,&viewer); CHKERRQ(ierr);
...
PetscErrorCode  ComputePressureAtEmissionTime(Vec P, Vec Pn, Vec Pt, Vec Tauq){
    ...
    while ((c = fgetc(f1)) != EOF)
    {
        ...
        if (t > emission_begin && t < emission_end){
          ...
          sprintf(filename1, "p-%05d.dat", it);
          sprintf(filename2, "p-%05d.dat", it+1);
...
int main(int argc, char **argv)
{
    ...
    Point     xo              =   {3.0, 3.0, 3.0};            //observer location
    PetscReal c               =   250.0;                      //acoustic wave speed
    PetscReal dt              =   0.0001;                     //time step
    PetscReal h               =   0.05;                        //cell size
    PetscInt  Nt              =   1000;                        //No of timestep
    ...
    for (PetscInt it = 0; it < Nt; ++it){

        PetscReal to = it*dt;   //observer time
		    ierr = PetscPrintf(PETSC_COMM_WORLD, "it = %d  to = %f\n", it, to); CHKERRQ(ierr);
        ...
        ierr = PetscPrintf(PETSC_COMM_WORLD, "2 Compute p, pn, pt at emission time tau\n"); CHKERRQ(ierr);
        ...
        // Compute Kirchhoff Integral
        //--------------------------------------------------------------
        ierr = PetscPrintf(PETSC_COMM_WORLD, "4 Writing Pressure at observer location xo and observer time to\n\n"); CHKERRQ(ierr);
        if (MyPID == 0) {
