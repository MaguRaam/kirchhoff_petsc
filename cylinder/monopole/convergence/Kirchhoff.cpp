...
void Kirchhoff::compute_pressure_at_emission_time(SphericalWave wave){
	...
	for (boost::multi_array<double, 3>::index i = 0; i < Ncells_r; ++i){
		for (boost::multi_array<double, 3>::index j = 0; j < Ncells_theta; ++j){
			...
			for (boost::multi_array<double, 3>::index q = 0; q < nqpts; ++q){
				...
				//get radius and radius vector:
				Point p{xq_top[i][j][q][0], xq_top[i][j][q][1], xq_top[i][j][q][2]};
				
				double radius_mag = norm(p);
				Vector<double> radius_vec = normalize(p);
				Vector<double> n{nq_top[i][j][q][0], nq_top[i][j][q][1], nq_top[i][j][q][2]};


				pq_top[i][j][q] = wave.p(radius_mag, tauq_top[i][j][q]);
				pnq_top[i][j][q] = dot(wave.dpdr(radius_mag, tauq_top[i][j][q])*radius_vec, n);
				ptq_top[i][j][q] = wave.dpdt(radius_mag, tauq_top[i][j][q]);
				...
				//get radius and radius vector:
				p = Point{xq_bottom[i][j][q][0], xq_bottom[i][j][q][1], xq_bottom[i][j][q][2]};
				
				radius_mag = norm(p);
				radius_vec = normalize(p);
				n = Vector<double>{nq_bottom[i][j][q][0], nq_bottom[i][j][q][1], nq_bottom[i][j][q][2]};

				pq_bottom[i][j][q] = wave.p(radius_mag, tauq_bottom[i][j][q]);
				pnq_bottom[i][j][q] = dot(wave.dpdr(radius_mag, tauq_bottom[i][j][q])*radius_vec, n);
				ptq_bottom[i][j][q] = wave.dpdt(radius_mag, tauq_bottom[i][j][q]);
	...
	for (boost::multi_array<double, 3>::index j = 0; j < Ncells_theta; ++j){
		for (boost::multi_array<double, 3>::index k = 0; k < Ncells_z; ++k){
			...
			for (boost::multi_array<double, 3>::index q = 0; q < nqpts; ++q){
				...
				//get radius and radius vector:
				Point p{xq_curved[j][k][q][0], xq_curved[j][k][q][1], xq_curved[j][k][q][2]};
				
				double radius_mag = norm(p);
				Vector<double> radius_vec = normalize(p);
				Vector<double> n{nq_curved[j][k][q][0], nq_curved[j][k][q][1], nq_curved[j][k][q][2]};

				pq_curved[j][k][q] = wave.p(radius_mag, tauq_curved[j][k][q]);
				pnq_curved[j][k][q] = dot(wave.dpdr(radius_mag, tauq_curved[j][k][q])*radius_vec, n);
				ptq_curved[j][k][q] = wave.dpdt(radius_mag, tauq_curved[j][k][q]);
