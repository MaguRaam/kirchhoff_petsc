...
void Kirchhoff::compute_pressure_at_emission_time()
{
	...
	for (int n = 0; n < t.size() - 2; ++n){
		...
		//TODO the filename style should exactly match flow solver data style otherwise it will throw pressure file not found error:

		//read top data:
		sprintf(current_file, "input/top/p-%05d.dat", file_index[n]);
		sprintf(next_file, "input/top/p-%05d.dat", file_index[n + 1]);
		read_pressure_data(current_file, p_top1, pz_top1);
		read_pressure_data(next_file, p_top2, pz_top2);

		//read bottom data:
		sprintf(current_file, "input/bottom/p-%05d.dat", file_index[n]);
		sprintf(next_file, "input/bottom/p-%05d.dat", file_index[n + 1]);
		read_pressure_data(current_file, p_bottom1, pz_bottom1);
		read_pressure_data(next_file, p_bottom2, pz_bottom2);

		//read curved data:
		sprintf(current_file, "input/curved/p-%05d.dat", file_index[n]);
		sprintf(next_file, "input/curved/p-%05d.dat", file_index[n + 1]);
...
int main()
{
	...
	const int Ncells_r = 10;
	const int Ncells_theta = 62;
	const int Ncells_z = 300;
